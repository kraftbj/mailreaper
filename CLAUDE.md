...
**LLM layer** (`llm/adapter.js`): Gemini and Ollama backends behind a unified interface. Both request JSON responses. `llm/prompts.js` has the prompt templates. LLM verdicts are cached in `browser.storage.local` with 24h TTL, max 1000 entries.
